...
import { Cloud, HardDrive, ImagePlus, Loader2, Lock, LogOut, Server, Trash2 } from "lucide-react";
...
const STORAGE_OPTIONS: {
...
}[] = [
  {
    value: "supabase",
    label: "Supabase",
    description: "Best for images under 50 MB",
    icon: <Server size={14} />,
  ...
  },
  {
    value: "r2",
    label: "Cloudflare R2",
    description: "No size limit, fast delivery",
    icon: <Cloud size={14} />,
  ...
  },
  {
    value: "drive",
    label: "Google Drive",
    description: "15 GB free storage",
...
const AdminGallery = () => {
  ...
  const [categoryOptions, setCategoryOptions] = useState<string[]>(() =>
    mergeCategoryOptions(GALLERY_CATEGORIES)
  );
  const [category, setCategory] = useState<string>(() =>
    getDefaultCategory(mergeCategoryOptions(GALLERY_CATEGORIES))
  );
  ...
  const handleDelete = async (item: GalleryItem) => {
    const confirmed = window.confirm(
      `Delete "${item.title}" from gallery? This cannot be undone.`
    );
    if (!confirmed) return;
    setActionError("");
    try {
              ...
              {items.length === 0 ? (
                ...
                <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-3">
                  {items.map((item) => (
                    <div
                      key={item.id}
                      className="rounded-lg border border-border p-2 flex flex-col gap-2"
                    >
                      <img
                        src={item.imageUrl}
                        alt={item.title}
